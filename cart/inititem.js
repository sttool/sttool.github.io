$(document).ready(function(){
	...
	function setgrp(grpid, grparr){
		...
		for(i = 0; i < (grparr.length-1)/3; i++){	
			...
			grp +='</h4><p data-name="product_desc">' + grparr[arrcounter++] + '</p> <div> <div class="form-group2"> <input class="sc-cart-item-qty" name="product_quantity" min="1" value="1" type="number"> <input name="product_price" value="';
			
			grp += grparr[arrcounter++] + '" type="hidden" /> <input name="product_id" value="12" type="hidden" /> <button class="sc-add-to-cart btn btn-success btn-sm pull-right">Add to cart</button> </div> </div> <div class="clearfix"></div> </div> </div> </div>';
		}
